...
ameacadas <- read.csv("floraAmeacadas.csv")
fauna <- read.csv("faunaAmeacada.csv")

# Carrega os Módulos  --------------------------------------------

source("envia_arquivo.R")
source("compara_arquivo.R")
...
ui <- fluidPage(
    ...
    sidebarPanel(
        ...
        conditionalPanel(
            ...
            ArqEntrada_UI(id = "mod_inventario", label = "Planilha (.xlsx ou .csv) com as espécie do inventário"),
        ...
        conditionalPanel(
            ...
            # Baixar tabela
            downloadButton("downloadData", "Baixar a tabela comparativa"),
    ...
    mainPanel(
        tabsetPanel(
            ...
            tabPanel("Comparativo", TabCompara_UI(id = "comparativo")),
...
server <- function(input, output, session) {
  ...
  # Carrega o Módulo com a planilha enviada 
  {
    ...
    tabInventario <- callModule(module = ArqEntrada, id = "mod_inventario", stringsAsFactors = FALSE)
    ...
    observeEvent(tabInventario$trigger, {
      req(tabInventario$trigger>0)
      rv$variable    <- tabInventario$variable
      rv$extensao <- c()
    })
  }
  tabInventario <- callModule(module = ArqEntrada, id = "mod_inventario", stringsAsFactors = FALSE)
    ...
    # Tabela comparativa ------------------------------------------------------
    callModule(module = TabCompara, id = "comparativo")
    
    # verAmeacadas <- callModule(module = TabCompara, id = "ver_ameacadas", stringsAsFactors = FALSE)
    # 
    ...
    output$downloadData <- downloadHandler(
        ...
        content = function(file) {
            ...
            write.csv(verAmeacadas(), file, row.names = FALSE)
        }
    )
    ...
    output$ref <- renderUI({
        ...
        tags$div(class = "conteudo", style = "",
                 ...
                 tags$div("Este aplicativo foi elaborado com o ambiente de programação", tags$cite("R"),tags$sup("1"),",",
                          ...
                          tags$cite("dplyr"),tags$sup("3"),",",
                          tags$cite("stringr"),tags$sup("4"),"e",
                          tags$cite("readxl"),tags$sup("5"),"e",
                 ...
                 tags$p(tags$b("3"),"Hadley Wickham, Romain François, Lionel Henry e Kirill Müller (2020). dplyr: A Grammar of Data Manipulation. R package version 0.8.4.",
                        tags$a(href = "https://CRAN.R-project.org/package=dplyr", "https://CRAN.R-project.org/package=dplyr", target = "_blank")
                 ),
                 
                 tags$p(tags$b("4"),"Hadley Wickham (2019). stringr: Simple, Consistent Wrappers for Common String Operations. R package version 1.4.0.",
                        tags$a(href = "https://CRAN.R-project.org/package=stringr", "https://CRAN.R-project.org/package=stringr", target = "_blank")
                 ),
                 
                 tags$p(tags$b("5"),"Hadley Wickham and Jennifer Bryan (2019). readxl: Read Excel Files. R package version 1.3.1.",
                        tags$a(href = "https://CRAN.R-project.org/package=readxl", "https://CRAN.R-project.org/package=readxl", target = "_blank")
                 ),
